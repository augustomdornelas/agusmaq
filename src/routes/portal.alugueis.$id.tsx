...
function AluguelDetail() {
  ...
  function addPg() {
    ...
    addPagamento({ aluguel_id: aluguel.id, data: pgData, valor: pgValor, forma: pgForma, observacao: pgObs });
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-lg bg-white p-5 shadow-sm lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
            <Info label="Início" value={dateBR(aluguel.data_inicio)} />
            <Info label="Devolução prevista" value={dateBR(aluguel.data_prevista_devolucao)} />
            <Info label="Devolução real" value={dateBR(aluguel.data_devolucao_real)} />
            <Info label="Cobrança" value={aluguel.tipo_cobranca} />
          </div>
          ...
          <div className="mt-3 flex justify-end gap-6 text-sm">
            <div><span className="text-[#6E7280]">Desconto: </span><b>−{money(aluguel.desconto)}</b></div>
            <div><span className="text-[#6E7280]">Frete: </span><b>+{money(aluguel.valor_frete)}</b></div>
            <div className="text-base"><span className="text-[#6E7280]">Total: </span><b className="text-[#213368]">{money(aluguel.valor_total)}</b></div>
          </div>
        ...
        <div className="rounded-lg bg-white p-5 shadow-sm">
          ...
          <div className="mb-3 space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-[#6E7280]">Total</span><b>{money(aluguel.valor_total)}</b></div>
